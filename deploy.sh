...
echo "
Options : 
	- --create : lancer des conteneurs (ex : ./deploy.sh --create 2)
...
createNodes() {
	...
	idmax=`docker ps -a --format '{{ .Names}}' | awk -F "-" -v user="$USER" '$0 ~ user"-debian" {print $3}' | sort -rn | head -1`
	...
	for i in $(seq $min $max);do
		# on commence par récupérer l'image créée par le Dockerfile et uploadée sur le dockerhub
